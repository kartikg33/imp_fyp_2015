...
{\color{red}! MISSING IMAGE !}
%\includegraphics[scale = 0.2]{Images/paws_3boards_final}
\caption{Prototype 0.2.04 - Instability caused by Filtering in Interface}
\label{fig:badfilt}
\end{figure}
...
\begin{wrapfigure}[35]{r}{0.37\textheight}
\centering
\subfloat[No Finger Boards attached]{
\includegraphics[width=0.4\textheight, height=0.15\textheight]{Images/rail_noboards.eps}
\label{fig:rail_noboards}
}
\\
\subfloat[1 Finger Board attached]{
\includegraphics[width=0.4\textheight, height=0.15\textheight]{Images/rail_1board.eps}
\label{fig:rail_1board}
}
\\
\subfloat[3 Finger Boards attached]{
\includegraphics[width=0.4\textheight, height=0.15\textheight]{Images/rail_3boards.eps}
...
Figure~\ref{fig:rail_noboards} shows the power rail measured on the Arduino Shield with no Finger Boards connected. We can see that it is centred around 4.97V, instead of the expected 5V, and features approximately 20mV of digital noise ripple. After connecting one Finger Board to the Arduino, I found that the rail voltage dropped further to around 4.91V, a drop of 60mV, with the same noise ripple of 20mV. Figure~\ref{fig:rail_1board} shows the waveform of the rail voltage. We also see a periodic ripple overlaid onto the waveform with a frequency determined to be 490Hz. This is in fact the frequency of the PWM signal sent by the Arduino to the Finger Board to produce an analogue voltage that can control the gain of the Board's amplifier. This frequency has been induced onto the power rails, most likely through the cable that attaches the Arduino Shield to the Finger Board. 

Figure~\ref{fig:rail_3boards} shows the power rail with three Finger Boards connected. The voltage dropped significantly to 4.78V but the noise ripple remained constant at 20mV. For this measurement, the PWM frequency was changed down to 30Hz for the first Finger Board, and 61Hz for the second and third Finger Boards. The waveform does not show it but there is in fact an overlaid periodicity corresponding to approximately 60Hz. The power required by the three Finger Boards' microphones and amplifiers causes a large drain on the Arduino, resulting in the voltage drop of the power rail. 

Figure~\ref{fig:sig_1} shows the microphone signal from a Finger Board, measured at the analogue input pin on the Arduino Shield. The signal has been listed as being in an \textit{idle state}, meaning that no deliberate sounds were made so the microphone was only capturing the background noise (audio) of the labs and the electrical noise of the circuitry. The signal is centred around 2.41V as opposed to the expected 2.5V. We can see large spikes in the waveform due to the induction of the PWM signal. The frequency response plot confirms that the periodic spikes do in fact correspond to the 490Hz of the PWM signal. We can also see a high amount of energy around the 10kHz mark, most likely corresponding to thermal noise, which was previously found to be present and was dealt with using a simple low pass filter with a cut-off of 20kHz. Of course, the best solution to remove this noise would be isolating the microphone to reduce its thermal sensitivity. 

\begin{figure}[H]
\centering
\includegraphics[width=0.6\textheight, height=0.45\textheight]{Images/sig_1.eps}
\caption{Microphone Signal (idle state) with PWM Frequency at 490Hz, measured at the Arduino input pin}
\label{fig:sig_1}
\end{figure}

\begin{wrapfigure}[21]{r}{0.4\textheight}
\centering
...
Figure~\ref{fig:sig_samp} \\
{\color{red} ///// sampled signal of idle state and ring tap, take from queueInput}

\begin{figure}[H]
\centering
{\color{red}! MISSING IMAGE !}
%\includegraphics[width=0.6\textheight, height=0.45\textheight]{Images/sig_samp.eps}
\caption{Microphone Signal with PWM Frequency at 30Hz, received by the Interface program}
\label{fig:sig_samp}
\end{figure}
...
Part  & Quantity  &   Image \\
 \hline
 Finger Ring & 2 &   {\color{red}! MISSING IMAGE !}\\
 Thumb Ring & 1  &   {\color{red}! MISSING IMAGE !}  \\
 Finger Board &3   &    {\color{red}! MISSING IMAGE !}\\
 4-way Jack Cable &3   &      {\color{red}! MISSING IMAGE !}\\
 Arduino Shield &1 &          {\color{red}! MISSING IMAGE !}\\
 Arduino Uno &1    &          !{\color{red}! MISSING IMAGE !}\\
 USB Type A/B Cable &1 &          {\color{red}! MISSING IMAGE !}\\
\end{tabular}
\end{center}
