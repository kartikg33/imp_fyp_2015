...
def samplefunc(bus):
    ...
    while(SampleFl[bus] and ser[bus]!='NULL'):
        ...
        trig = 0
        index = diff
        while(not trig and index < N):
            #print index
            #print inbuff[index].value
            if((inbuff[index].value - inbuff[index-diff].value) >= thresh):
                #print trig
                trig = 1
            index+=1
        
        
        #trig = [1 for index in inbuff[diff:N] if index.value >= thresh]
        # if trig is all ones and readytoplay, play and make not readytoplay
        #if 1 in trig and readytoplay:
        if trig and readytoplay:
...
def auIn(bus):
    ...
    while(SampleFl[bus]):
        ...
        #print input #debugging
            try:
                #inbuff.append(Sig((int(input)-512)*GAIN))
                inbuff.append(Sig(int(input)*1.0))
            except ValueError:
                pass
        except serial.SerialException:
            pass
...
class MyPanel(wx.Panel):
    ...
    def SelectBus(self, e):
        ...
        try:
            ...
            #print len(self.boardSizer[bus].GetChildren())
            if len(self.boardSizer[bus].GetChildren())>1:
                self.boardSizer[bus].Hide(1)
                self.boardSizer[bus].Remove(1)
                self.frame.fSizer.Layout()
                self.frame.Fit()
            pass
